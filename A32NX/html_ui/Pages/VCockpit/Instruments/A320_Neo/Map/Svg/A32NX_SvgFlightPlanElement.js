    ...
    makePathFromWaypoints(waypoints, map) {
        ...
        if (points.length > 2) {
            ...
            for (let i = 1; i < points.length - 1; i++) {
                ...
                const kts = Math.max(SimVar.GetSimVarValue("GPS GROUND SPEED", "knots"), 220); // 220; // knots, i.e. nautical miles per hour
                const nms = kts / 60 / 60; // nautical miles per second

                const deg2rad = Math.PI / 180;
                const xs = (nms / 3) * (Math.tan(deg2rad * Math.abs(deltaPc / 2)) / deg2rad);
